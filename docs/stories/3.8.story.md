...
- [ ] **Task 1: Database Migration (AC: 1, 12)**
    - [ ] Create Alembic migration to add `is_public` boolean column to `cvs` table
    - [ ] Default value: `false` for existing CVs
    - [ ] Run migration and verify

- [ ] **Task 2: Update CV Model & Schema (AC: 1, 14)**
    - [ ] Add `is_public: Mapped[bool]` to `CV` model in `backend/app/modules/cv/models.py`
    - [ ] Update `CVResponse` and `CVWithStatusResponse` schemas to include `is_public`
    - [ ] Update shared-types: add `is_public` to CV types

- [ ] **Task 3: Create Visibility Update Endpoint (AC: 13)**
    - [ ] Add `PATCH /api/v1/cvs/{cv_id}/visibility` endpoint in `backend/app/modules/cv/router.py`
    - [ ] Request body: `{ "is_public": boolean }`
    - [ ] Validate CV ownership before update
    - [ ] Return updated CV response

- [ ] **Task 4: Create Recruiter CV Access Endpoint (AC: 8, 9)**
    - [ ] Add `GET /api/v1/jobs/jd/{jd_id}/candidates/{cv_id}` endpoint
    - [ ] Verify JD belongs to current user (recruiter)
    - [ ] Verify CV exists and has been matched with this JD
    ...
    - [ ] Return `CVAnalysis` response with owner info (name if available)

- [ ] **Task 5: Update CV List Frontend (AC: 2, 3, 4, 5, 6, 16)**
    - [ ] Update `frontend/features/cv/components/CVList.tsx` or create `CVVisibilityToggle.tsx`
    - [ ] Add toggle switch for each CV with current status
    - [ ] Show "Public" (green) or "Private" (gray) badge
    - [ ] Add tooltip explaining visibility
    - [ ] Confirmation dialog on toggle
    - [ ] Call `updateCVVisibility` action on change

- [ ] **Task 6: Create CV Visibility Actions (AC: 2, 6)**
    - [ ] Add `updateCVVisibilityAction(cvId: string, isPublic: boolean)` to `frontend/features/cv/actions.ts`
    - [ ] Add `updateCVVisibility` to `frontend/services/cv.service.ts`
    - [ ] Handle errors and return result

- [ ] **Task 7: Update CandidateCard Component (AC: 7, 10)**
    - [ ] Update `frontend/features/jobs/components/CandidateCard.tsx`
    - [ ] Add `is_public` field to `RankedCandidateResponse` type
    - [ ] Show visibility indicator (lock icon for private, checkmark for public)
    - [ ] Update "Xem CV" button:
        - If public: Link to `/jobs/jd/${jdId}/candidates/${cvId}`
        - If private: Show disabled button with tooltip "CV is private"

- [ ] **Task 8: Create Recruiter CV Detail Page (AC: 7, 18)**
    - [ ] Create `frontend/app/jobs/jd/[jdId]/candidates/[cvId]/page.tsx`
    - [ ] Fetch CV analysis via new endpoint
    - [ ] Display read-only CV analysis (reuse CVAnalysisResults component)
    - [ ] Show match context (how this CV matches the JD)
    - [ ] Back button to candidate list
    - [ ] Handle 403 error with appropriate message

- [ ] **Task 9: Update Candidate Ranking to Include Visibility (AC: 10, 11)**
    - [ ] Update `CandidateRanker` to include `is_public` in response
    - [ ] Update `RankedCandidateResponse` schema to include `is_public`
    - [ ] Update shared-types accordingly

- [ ] **Task 10: Write Unit Tests (AC: 19)**
    - [ ] Test visibility toggle component
    - [ ] Test CandidateCard with public/private states
    - [ ] Test recruiter CV access endpoint (public CV, private CV, not owner)
    - [ ] Test visibility update endpoint

- [ ] **Task 11: Manual Testing**
    - [ ] Test job seeker changing visibility
    - [ ] Test recruiter viewing public CV
    - [ ] Test recruiter attempting to view private CV
    - [ ] Test persistence of visibility setting
...
backend/
├── alembic/versions/
│   └── xxxx_add_is_public_to_cvs.py     # New migration
├── app/modules/cv/
│   ├── models.py                         # Add is_public field
│   ├── schemas.py                        # Update response schemas
│   └── router.py                         # Add visibility endpoint
└── app/modules/jobs/
    └── router.py                         # Add recruiter CV access endpoint
```

**Frontend Files to Modify:**
```
frontend/
├── app/jobs/jd/[jdId]/candidates/
│   └── [cvId]/
│       └── page.tsx                      # New: Recruiter CV detail page
├── features/cv/
│   ├── actions.ts                        # Add visibility actions
│   └── components/
│       └── CVVisibilityToggle.tsx        # New component
├── features/jobs/
│   └── components/
│       └── CandidateCard.tsx             # Update with visibility indicator
├── services/
│   └── cv.service.ts                     # Add visibility update method
└── types/
    └── index.ts                          # Update CV types
```

**Shared Types:**
```
packages/shared-types/src/
├── cv.ts                                 # Add is_public to CVWithStatus
└── job.ts                                # Add is_public to RankedCandidateResponse
```
...
2. **Data Exposure:**
   ...
   - File download should follow same visibility rules
...
|      Senior Python developer with 4+ years...      ✓ Public |
|      Matched: [Python] [FastAPI] [PostgreSQL]              |
|                                           [Xem CV]          |
+-------------------------------------------------------------+
| #2   private_cv.pdf                               [85%]    |
|      Mid-level developer...                        🔒 Private|
