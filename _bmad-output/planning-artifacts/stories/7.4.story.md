...
- [x] **Typing Indicator Events** (AC: 6, 7, 8)
  - [x] Add `typing-start` event handler in `socket-server/handlers/message.js`
  - [x] When received, broadcast `user-typing` event to conversation room (exclude sender)
  - [x] Event payload: `{ conversation_id, user_id, user_name }`
  - [x] Add `typing-stop` event handler
  - [x] When received, broadcast `user-stopped-typing` event to conversation room
  - [x] Event payload: `{ conversation_id, user_id }`

- [x] **Typing Indicator Auto-Timeout** (AC: 8)
  - [x] Implement server-side timeout tracking
  - [x] Store typing users in memory: `Map<conversationId, Map<userId, timeoutId>>`
  - [x] Set 5-second server timeout when `typing-start` received
  - [x] Clear timeout if `typing-stop` received before timeout
  - [x] Auto-emit `user-stopped-typing` if timeout expires
  - [x] Prevents stuck typing indicators if client disconnects

- [x] **Connection Status Events** (AC: 9, 10)
  - [x] Socket.io automatically handles `connect` and `disconnect` events
  - [x] No additional backend code needed
  - [x] Frontend listens to these built-in events

- [x] **Optimize Event Broadcasting** (AC: 7)
  - [x] Typing events should NOT be persisted to database
  - [x] Use in-memory broadcasting only
  - [x] Target latency: <200ms for typing indicator

### Frontend Tasks - Toast Notifications

- [x] **Toast Notification Library Setup** (AC: 1, 2, 3)
  - [x] Install Sonner library: `npm install sonner`
  - [x] Add `<Toaster />` component to `frontend/app/layout.tsx`
  - [x] Configure toast position: bottom-right
  - [x] Configure toast duration: 4 seconds

- [x] **Global Message Notification Listener** (AC: 1, 2, 3)
  - [x] Extend `useGlobalSocket` hook from Story 7.3
  - [x] Listen to `new-message` event from user room (`user:${userId}`)
  - [x] Check if user is currently on the conversation page
  - [x] If NOT on conversation page, show toast notification
  - [x] Toast content: `{sender_name}: {message_content.substring(0, 50)}...`
  - [x] Toast action: onClick navigate to `/messages/${conversationId}`

- [x] **Smart Toast Display Logic** (AC: 1)
  - [x] Use `usePathname()` from Next.js to get current route
  - [x] Don't show toast if on `/messages/${conversationId}` (already seeing message)
  - [x] Show toast if on different conversation or different page entirely
  - [x] Show toast if user is on conversation list (`/messages`) but not in specific conversation

### Frontend Tasks - Unread Count Badge

- [x] **Navigation Badge Component** (AC: 4, 5)
  - [x] Create `frontend/components/layout/MessagesBadge.tsx`
  - [x] Use conversation list context from Story 7.3
  - [x] Calculate total unread: `conversations.reduce((sum, conv) => sum + conv.unread_count, 0)`
  - [x] Display red badge with number if unread count > 0
  - [x] Hide badge if unread count === 0
  - [x] Max display: 99+ (if count > 99, show "99+")

- [x] **Real-time Badge Updates** (AC: 5)
  - [x] Badge automatically updates when conversation list context changes
  - [x] Context updates via `conversation-updated` Socket.io event (from Story 7.3)
  - [x] Badge decrements when user opens conversation (mark as read from Story 7.3)
  - [x] No additional Socket.io events needed
...
- [x] **Typing Detection Hook** (AC: 6, 7, 8)
  - [x] Create `frontend/hooks/useTypingIndicator.ts`
  - [x] Track local typing state (is current user typing?)
  - [x] Track remote typing state (is other user typing?)
  - [x] Emit `typing-start` when user starts typing (debounced 300ms)
  - [x] Emit `typing-stop` when user stops typing (debounced 3 seconds)
  - [x] Listen to `user-typing` event to show indicator
  - [x] Listen to `user-stopped-typing` event to hide indicator

- [x] **Typing Indicator Component** (AC: 6, 7, 8)
  - [x] Create `frontend/features/messages/components/TypingIndicator.tsx`
  - [x] Display: "{user_name} đang nhập..." with animated dots
  - [x] Animate dots: "..." → ".  " → " . " → "  ." (cycle every 500ms)
  - [x] Position above message input area
  - [x] Show only when remote typing state is true
  - [x] Hide with fade-out animation (200ms transition)

- [x] **Message Input Integration** (AC: 6, 7, 8)
  - [x] Update `MessageInput.tsx` to use `useTypingIndicator` hook
  - [x] On input onChange:
  ...
  - [x] On input empty or 3 seconds no typing, emit `typing-stop`
  - [x] On message send, emit `typing-stop` immediately
  - [x] Clear local typing state when message sent
...
- [x] **Connection Status Component** (AC: 9, 10)
  - [x] Create `frontend/features/messages/components/ConnectionStatus.tsx`
  - [x] Display small badge: "Đã kết nối" (green) or "Mất kết nối" (red)
  - [x] Use Socket.io `isConnected` state from `useSocket` hook
  - [x] Position: Top-right corner of chat window or conversation list
  - [x] Auto-hide "Connected" badge after 2 seconds (only show on reconnect)
  - [x] Always show "Disconnected" badge until reconnected

- [x] **Socket Connection State Tracking** (AC: 9, 10)
  - [x] Extend `useSocket` hook to expose `isConnected` state
  - [x] Listen to Socket.io `connect` event → set isConnected = true
  - [x] Listen to Socket.io `disconnect` event → set isConnected = false
  - [x] Listen to Socket.io `connect_error` event → set isConnected = false
  - [x] Provide connection status to ConnectionStatus component
...
- Socket server typing events: `socket-server/server.js:169-242`
- Typing indicator tests: `socket-server/tests/connection.test.js:239-334`
- Toast notification hook: `frontend/hooks/useGlobalSocket.ts:48-66`
- Typing indicator hook: `frontend/hooks/useTypingIndicator.ts`
- Connection status component: `frontend/features/messages/components/ConnectionStatus.tsx`
- Conversation context: `frontend/features/messages/contexts/ConversationContext.tsx`
...
1. **Backend Socket.io Events:**
   - Server-side typing timeout tracking (5s auto-timeout)
   - Enhanced typing events with proper payload format
   - Cleanup on disconnect to prevent stuck indicators

2. **Frontend Toast Notifications:**
   - Sonner library integration (bottom-right, 4s duration)
   - Smart display logic (only show when not on conversation page)
   - Click action to navigate to conversation

3. **Navigation Unread Badge:**
   - Real-time badge updates via ConversationContext
   - 99+ display for high counts
   - Auto-hide when unread count = 0

4. **Typing Indicators:**
   - Debounced typing events (300ms start, 3s stop)
   - Animated dot indicators with Vietnamese text
   - Server-side timeout protection

5. **Connection Status:**
   - Real-time connection state tracking
   - Auto-hide "Connected" after 2s
   - Persistent "Disconnected" until reconnected

**🔧 Technical Decisions:**
- Used existing useSocket hook's isConnected state
- Enhanced conversation-updated event to include sender_name
- Implemented proper TypeScript interfaces for all components
- Followed existing project patterns for file organization

**🧪 Test Coverage:**
- All Socket.io typing events tested
- Server-side timeout tracking verified
- Disconnect cleanup confirmed

All acceptance criteria met. Story ready for code review.
...
- `socket-server/server.js` - Enhanced typing indicator events with server-side timeout tracking
- `socket-server/tests/connection.test.js` - Added comprehensive typing indicator tests

**Frontend Files Modified:**
- `frontend/components/ui/sonner.tsx` - Updated toast position and duration
- `frontend/hooks/useGlobalSocket.ts` - Already included toast notification logic
- `frontend/components/layout/MessagesBadge.tsx` - Real-time unread count badge
- `frontend/features/messages/components/MessageInput.tsx` - Updated typing event props

**Frontend Files Created:**
- `frontend/hooks/useTypingIndicator.ts` - Typing detection and event management
- `frontend/features/messages/components/TypingIndicator.tsx` - Animated typing indicator UI
- `frontend/features/messages/components/ConnectionStatus.tsx` - Connection status badge
- `frontend/features/messages/contexts/ConversationContext.tsx` - Conversation state management

**Dependencies:**
- All dependencies already existed (sonner, socket.io-client, etc.)

**Configuration:**
- No environment variable changes required
- Used existing socket server setup from Stories 7.1-7.3
